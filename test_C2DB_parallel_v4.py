...
client = MongoClient('localhost', 6007)
	...
	for inter_defect in InterstitialGenerator(struct, element):
		...
		for charged_defect in SimpleChargeGenerator( inter_defect):
			...
			if int_type=="extrinsic":
				charged_inter_data= open('ext-%s-chg(%s)-def_cnt(%s)-def_chrg_cnt(%s).cif'%(inter_defect.name,charged_defect.charge,cnt_defect,cnt_defect_chg),'w')
				#charged_inter_data= open('ext-%s-chg(%s).cif'%(inter_defect.name,charged_defect.charge),'w')
			elif int_type=="intrinsic":
				charged_inter_data= open('int-%s-chg(%s)-def_cnt(%s)-def_chrg_cnt(%s).cif'%(inter_defect.name,charged_defect.charge,cnt_defect,cnt_defect_chg),'w')
...
def find_compounds(uid):
	print(uid)
	query = collection.find({"c2db_uid":"%s"%(uid)})
	elements=[]
	for doc in query:
		chem_form = [doc["c2db_uid"]]
		struc = [doc["structure"]]
		struc_id = [doc["_id"]]
		struc_id = struc_id[0]
...
def query():
	...
	formulas_done=['Ta2Br2N2-FeOCl-NM',
'Ti2Br2Se2-FeOCl-NM',
'Ti2I2O2-FeOCl-NM',
'Pt2I2Se2-FeOCl-NM',
'Sc2Cl2Se2-FeOCl-NM',
